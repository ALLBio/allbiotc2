...
class VariationStatistics:
	...
	def __init__(self, chromosome, index, typing=None):
		...
		self.index = index
		self.typing = typing
		# List of (similar) annotations: (index,length_diff,offset,genotype)
		self.hits = []
	def add_hit(self, index, length_diff, offset, typing=None):
...
#				typing = alltypedict[genotype]
	...
	def get(self, chromosome, index):
		...
		coord1, coord2, var_type, tags = self.variations[chromosome][index]
		if len(tags) == 0:
			tags = set([None])
		return coord1, coord2, var_type, tags
	...
	def find_all_deletion_overlaps(self, another_list, chromosome, trio=False, min_length=None, max_length=None, offset=0, difflength=0, tag=None):
		...
		for n, (start, end, var_type, tags) in enumerate(another_list.variations[chromosome]):
			...
			events.append((start-offset, 'S', 1, n))
			events.append((end+offset, 'E', 1, n))
			#events.append(((end + start)/2), 1, n))
		events.sort()
...
#					print(tags0)
				...
				for j in overlaps[i]:
					...
					if mode == 'overlap':
						if (abs(length0-length1) <= difflength) and (overlap > 0):
							stats.add_hit(j, abs(length0 - length1), abs(center0 - center1), typing1)
					elif mode == 'fixed_distance':
						if (abs(length0-length1) <= difflength) and (abs(center0-center1) <= offset):
							stats.add_hit(j, abs(length0 - length1), abs(center0 - center1), typing1)
					elif mode == 'significant':
						assert 0.0 < siglevel < 1.0
						sig = callsignificance(length0, abs(center0-center1), abs(length0-length1), chromosome, count_stats) 
						if (sig <= siglevel):
							stats.add_hit(j, abs(length0 - length1), abs(center0 - center1), typing1)
					else:
...
#		print(varnumber,len(stats_list),file=sys.stderr)
	...
	def insertion_distances(self, another_list, chromosome, trio=False, min_length=None, max_length=None, offset=None, difflength=None, tag=None):
		...
		events = []
		lengths = []
		for n, (coord1, coord2, var_type, tags) in enumerate(self.variations[chromosome]):
		...
		for n, (coord1, coord2, var_type, tags) in enumerate(another_list.variations[chromosome]):
			# semantics for insertions...
			pos = coord1
			length = coord2
			if var_type != 'INS': continue
			if length <= 0: continue
			if difflength == None: difflength = 0
			if (min_length != None) and (length < max(5, min_length-difflength)): continue
			if (max_length != None) and (length > max_length+difflength): continue
			events.append((pos-offset, 'S', 1, n))
			events.append((pos+length+offset, 'E', 1, n))
		events.sort()
		results = defaultdict(set)
		active = [set(), set()]
		for pos, event_type, list_idx, variation_index in events:
	...
	def insertion_hit_statistics(self, another_list, chromosomes, count_stats, mode='overlap', trio=False, min_length=None, max_length=None, offset=0, difflength=0, siglevel=0.0, tag=None):
		...
		for chromosome in (x.lower() for x in chromosomes):
			...
			for i, (pos0, length0, var_type0, tags0) in enumerate(self.variations[chromosome]):
				if var_type0 != 'INS': continue
	#			print("Self ", chromosome, pos0, length0, len(overlaps[i]), file=sys.stderr)
				if (tag != None) and (not tag in tags0) and not trio: continue
				start0 = pos0
				end0 = pos0 + length0
				if (min_length != None) and (length0 < min_length): continue
	...
	#			print("Self ", chromosome, pos0, length0, len(overlaps[i]), file=sys.stderr)
				for j in overlaps[i]:
					pos1, length1, var_type1, tags1 = another_list.variations[chromosome][j]
					start1 = pos1
					end1 = pos1 + length1
					overlap = max(0, min(end0,end1) - max(start0,start1))
					dist = abs(pos0-pos1)
					if mode == 'overlap':
						if (overlap > 0) and (abs(length0-length1) <= difflength):
							stats.add_hit(j, abs(length0 - length1), dist)
					elif mode == 'fixed_distance':
						if (dist <= offset) and (abs(length0-length1) <= difflength):
							stats.add_hit(j, abs(length0 - length1), dist)
					elif mode == 'significant':
						assert 0.0 < siglevel < 1.0
						sig = callsignificance(length0, dist, abs(length0-length1), chromosome, count_stats) 
						if (sig <= siglevel):
							stats.add_hit(j, abs(length0 - length1), dist)
					else:
						print("Error: invalid mode \"%s\"."%mode, file=sys.stderr)
...
#	print(typematrix)
	return hits, hit_rate, avg_length_diff, avg_offset, hit_bitarray, typematrix, variantsums, callsums
...
def aggregate_toolwise_statistics(stats_lists, tool_names):
	...
	for name, (ins_prec_list, ins_recall_list, del_prec_list, del_recall_list) in zip(tool_names,stats_lists):
		...
		hits, hit_rate, avg_length_diff, avg_offset, hit_bitarray, typematrix, varsums, callsums = summarize_variation_stats_list(ins_prec_list)
...
#		print(hit_rate,file=sys.stderr)
		...
		hits, hit_rate, avg_length_diff, avg_offset, hit_bitarray, typematrix, varsums, callsums = summarize_variation_stats_list(ins_recall_list)
...
#		print(hit_rate,file=sys.stderr)
		...
		hits, hit_rate, avg_length_diff, avg_offset, hit_bitarray, typematrix, varsums, callsums = summarize_variation_stats_list(del_prec_list)
		...
		hits, hit_rate, avg_length_diff, avg_offset, hit_bitarray, typematrix, varsums, callsums = summarize_variation_stats_list(del_recall_list)
...
def print_results_ascii(results, length_ranges, true_indel_counts, print_typing):
	...
	wt = w0 + 15*w
	print('='*wt)
	print(' '*w0, format_range('Insertions', 6*w),' '*w, format_range('Deletions', 8*w), sep='')
	if print_typing:
		print(right_pad('Tool',w0), left_pad('Prec.',w), left_pad('Recall',w), left_pad('Excl.',w), left_pad('F',w), left_pad('Len.Diff.',w), left_pad('Dist.',w), ' '*w,left_pad('Prec.',w), left_pad('Ind.Prec.',w), left_pad('GT.Prec.',w), left_pad('Recall',w), left_pad('Excl.',w), left_pad('F',w), left_pad('Len.Diff.',w), left_pad('Dist.',w), sep='')
	else:
		print(right_pad('Tool',w0), left_pad('Prec.',w), left_pad('Recall',w), left_pad('Excl.',w), left_pad('F',w), left_pad('Len.Diff.',w), left_pad('Dist.',w), ' '*w,left_pad('Prec.',w), left_pad('Recall',w), left_pad('Excl.',w), left_pad('F',w), left_pad('Len.Diff.',w), left_pad('Dist.',w), sep='')
	for length_range,true_indels,toolwise_results in zip(length_ranges,true_indel_counts,results):
		print('-'*wt)
		print('Length range %d-%d'%length_range,'(insertions: %d, deletions %d)'%true_indels)
		for tool_stats in toolwise_results:
			if print_typing:
				...
				print(right_pad(tool_stats.name,w0), format_value(tool_stats.insertion_precision,w), format_value(tool_stats.insertion_recall,w), format_value(tool_stats.insertion_exclusivity,w), format_value(tool_stats.insertion_f,w), format_value(tool_stats.insertion_avg_lendiff,w,1.0), format_value(tool_stats.insertion_avg_distance,w,1.0),' '*w,\
					format_value(tool_stats.deletion_precision,w), format_value(deletion_ind_prec,w), format_value(deletion_gt_prec,w), format_value(tool_stats.deletion_recall,w), format_value(tool_stats.deletion_exclusivity,w), format_value(tool_stats.deletion_f,w), format_value(tool_stats.deletion_avg_lendiff,w,1.0), format_value(tool_stats.deletion_avg_distance,w,1.0), sep='')
			else:
				print(right_pad(tool_stats.name,w0), format_value(tool_stats.insertion_precision,w), format_value(tool_stats.insertion_recall,w), format_value(tool_stats.insertion_exclusivity,w), format_value(tool_stats.insertion_f,w), format_value(tool_stats.insertion_avg_lendiff,w,1.0), format_value(tool_stats.insertion_avg_distance,w,1.0),' '*w,\
					format_value(tool_stats.deletion_precision,w), format_value(tool_stats.deletion_recall,w), format_value(tool_stats.deletion_exclusivity,w), format_value(tool_stats.deletion_f,w), format_value(tool_stats.deletion_avg_lendiff,w,1.0), format_value(tool_stats.deletion_avg_distance,w,1.0), sep='')
	print('='*wt)
...
def print_results_latex(results, length_ranges, true_indel_counts):
	...
	print('\\begin{longtable}{lrrrrrr@{\\qquad}rrrrrr}')
	print('\\hline')
	print(' '*w0,' & \\multicolumn{6}{c}{\\textbf{Insertions}} & \\multicolumn{6}{c}{\\textbf{Deletions}} \\\\', sep = '')
	print(' '*w0, left_pad('Prec.',w), left_pad('Rec.',w), left_pad('Exc.',w), left_pad('F.',w), left_pad('$\Delta$Len.',w), left_pad('Dist.',w),\
		left_pad('Prec.',w), left_pad('Rec.',w), left_pad('Exc.',w), left_pad('F.',w), left_pad('$\Delta$Len.',w), left_pad('Dist.',w), sep=' & ', end='\\\\\n')
	print('\hline')
	for length_range,true_indels,toolwise_results in zip(length_ranges,true_indel_counts,results):
		print('\\multicolumn{13}{l}{\\textbf{Length Range %d--%d}'%length_range, '(%s true ins., %s true del.)}\\\\'%(format(true_indels[0],',d'),format(true_indels[1],',d')))
		best = ToolStatistics()
		for tool_stats in toolwise_results:
			best.improve(tool_stats)
		for tool_stats in toolwise_results:
			print(
				right_pad(tool_stats.name,w0),
...
def print_tag_table(stats_list, variants0, variants1, all_tags0, all_tags1, title, caption_total, caption_wrong, latex_output):
	...
	for v in stats_list:
		coord1, coord2, var_type, from_tags = variants0.get(v.chromosome, v.index)
		for from_tag in from_tags:
			...
			if len(v.hits) == 0:
				...
				for index,length_diff,offset in v.hits:
					coord1, coord2, var_type, to_tags = variants1.get(v.chromosome, index)
...
#				     help="maximum difference in length allowed between centerpoints for merging calls")
	...
	for min_length, max_length in length_ranges:
		...
		for i,predictions in enumerate(predictions_lists):
			...
			ins_recall_list = true_variants.insertion_hit_statistics(predictions, chromosomes, true_insertion_stats, options.mode, min_length, max_length, options.offset, options.difflength, options.siglevel, None)
			ins_prec_list = predictions.insertion_hit_statistics(true_variants, chromosomes, true_insertion_stats, options.mode, min_length, max_length, options.offset, options.difflength, options.siglevel, None)
	...
	if options.latex_output:
		print('\\documentclass[12pt]{scrartcl}')
		...
		for range_idx, (min_length, max_length) in enumerate(length_ranges):
			def print_list(l):
				for i in range(len(l[0])):
					v = l[0][i]
					coord1, coord2, var_type, tags = true_variants.get(v.chromosome, v.index)
					...
					for tool_idx in range(len(tool_names)):
						if l[tool_idx][i].is_hit():
							...
							pred_coord1, pred_coord2, pred_var_type, pred_tags = predictions_lists[tool_idx].get(v.chromosome, index)
	...
	if options.toolwise_outfile != None:
		...
		for tool_idx, tool_name in enumerate(tool_names):
			...
			f = open(filename, 'w')
			print('#{0}_chrom {0}_coord1 {0}_coord2 {0}_var_type {0}_tag true_coord1 true_coord2 true_tag'.format(tool_name), file=f)
			for range_idx, (min_length, max_length) in enumerate(length_ranges):
				def print_list(l):
					for v in l:
						coord1, coord2, var_type, tags = predictions_lists[tool_idx].get(v.chromosome, v.index)
						print(v.chromosome, coord1+1, coord2, var_type, tag_string(tags), end='', file=f)
						if v.is_hit():
							index, length_diff, offset, typing = v.get_best_hit()
							true_coord1, true_coord2, true_var_type, true_tags = true_variants.get(v.chromosome, index)
